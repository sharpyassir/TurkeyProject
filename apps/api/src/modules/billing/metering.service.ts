...
export class MeteringService {
  ...
  async tickFallback(now = new Date()) {
    ...
    const [servers, ips, snapshots] = await Promise.all([
      this.prisma.server.findMany({ where: { status: { in: ['active', 'off', 'rebooting', 'resizing', 'rebuilding'] }, meteredSince: { not: null } }, select: { id: true, projectId: true, hostId: true } }),
