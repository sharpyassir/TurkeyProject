...
export class PaymentsController {
  ...
  /** Stripe → us. Verified with Stripe-Signature. */
  @Public() @Post('payments/stripe/webhook') @HttpCode(200)
  async stripe(@Req() req: Request & { rawBody?: Buffer }, @Headers() headers: Record<string, string>) {
    await this.payments.handle('stripe', req.rawBody ?? Buffer.from(JSON.stringify(req.body ?? {})), headers);
    return { received: true };
  }

  /** iyzico → browser → us after the hosted page. We verify by retrieving the payment, then redirect. */
  @Public() @Post('payments/iyzico/callback')
  async iyzico(@Req() req: Request & { rawBody?: Buffer }, @Headers() headers: Record<string, string>, @Query() query: Record<string, string>, @Res() res: Response) {
    const raw = req.rawBody ?? Buffer.from(typeof req.body === 'string' ? req.body : new URLSearchParams(req.body ?? {}).toString());
    const out = await this.payments.handle('iyzico', raw, headers, query);
    const r = out[0];
  ...
  fakePage(@Query() q: Record<string, string>, @Res() res: Response) {
    const amount = new Intl.NumberFormat(q.currency === 'TRY' ? 'tr-TR' : 'en-US', { style: 'currency', currency: q.currency || 'USD' }).format(Number(q.amount || 0) / 100);
...
<body><form class="card" method="get" action="/v1/billing/payments/fake/confirm"><h1>Test payment page</h1><p>No real card is charged. This page stands in for Stripe or iyzico in development.</p><div class="amt">${esc(amount)}</div>
...
<button class="ok" name="outcome" value="ok">Pay ${esc(amount)}</button><button class="no" name="outcome" value="fail">Decline</button></form></body></html>`);
  ...
  async fakeConfirm(@Query() q: Record<string, string>, @Res() res: Response) {
    if (loadConfig().PAYMENT_PROVIDER_USD !== 'fake' && loadConfig().PAYMENT_PROVIDER_TRY !== 'fake') throw ApiError.notFound('page', 'fake');
