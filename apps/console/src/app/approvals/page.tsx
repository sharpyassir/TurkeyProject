...
export default function ApprovalsPage() {
  ...
  async function decide(a: Approval, decision: 'approve' | 'deny') {
    ...
    if (decision === 'deny') { const r = prompt('Tell the agent why (optional):'); if (r === null) return; reason = r || undefined; }
    if (decision === 'approve' && !confirm(`Approve and run now: ${a.summary}?`)) return;
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-neutral-500">Agent tokens with approval rules park these actions here. Nothing runs until a team owner or admin approves it. Requests expire after 24 hours. Set the rules per token under <Link href="/agents" className="text-blue-600 hover:underline">Agent Access</Link>.</p>
      </div>
      {error && <p className="rounded border border-red-200 bg-red-50 p-2 text-sm text-red-700 dark:bg-red-950/30">{error}</p>}

      <section className="space-y-3">
        <h2 className="font-medium">Waiting for you {pending.length > 0 && <span className="badge bg-amber-100 text-amber-800">{pending.length}</span>}</h2>
        {rows && pending.length === 0 && <p className="card text-sm text-neutral-500">Nothing is waiting. Agents can keep working within their scopes and spending caps.</p>}
        {pending.map((a) => (
          <div key={a.id} className="card space-y-2">
            ...
            <p className="text-sm text-neutral-500">
              Requested by token <span className="font-mono">{a.token?.name ?? 'deleted token'}</span> {new Date(a.createdAt).toLocaleString(locale)}.
              Expires {new Date(a.expiresAt).toLocaleString(locale)}.
              {a.resourceId && <> Server <Link href={`/servers/${a.resourceId}`} className="text-blue-600 hover:underline">{a.resourceName ?? a.resourceId}</Link>.</>}
            </p>
            ...
            <div className="flex gap-2">
              <button className="btn-primary" disabled={busy === a.id} onClick={() => decide(a, 'approve')}>Approve and run</button>
              <button className="btn-danger" disabled={busy === a.id} onClick={() => decide(a, 'deny')}>Deny</button>
            </div>
          </div>
        ))}
      </section>
