...
function CreateServerForm() {
  ...
  const estimate = (priceOf(size)?.monthlyMinor ?? 0) + (priceOf('public_ip')?.monthlyMinor ?? 0) + (app?.priceMonthlyMinor ?? 0);
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      await api('/v1/servers', { method: 'POST', idempotent: true, body: JSON.stringify({ name: f.get('name'), size, image, ...(app ? { appVariables } : {}) }) });
      router.push('/servers');
    } catch (err) {
